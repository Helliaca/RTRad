...
void gmain(triangle VSOut input[3], inout TriangleStream<GSOut> OutputStream)
{
    ...
    for (uint i = 0; i < 3; i += 1)
    {
        ...
        if (dominantAxis == faceNormal.x) output.posH = float4(output.posH.zyx, 1);
        else if (dominantAxis == faceNormal.y) output.posH = float4(output.posH.xzy, 1);
        else if (dominantAxis == faceNormal.z) output.posH = float4(output.posH.xyz, 1);
